<script>
	...
	function makeFollowRequest(id) {
		...
		if (!privateProfile) {
			$currentUserFollowing = [...$currentUserFollowing, { ...profile.user, status: 'accepted' }]
			followers = [...followers, $currentUser]
		} else if (privateProfile) {
			$currentUserFollowing = [...$currentUserFollowing, { ...profile.user, status: 'pending' }]
		}
		const data = {
	...
	let isFollowing = ''
	$: if ($currentUserFollowing) {
		const follower = $currentUserFollowing.find((follower) => follower.id === profile.user.id)
...
<div class="flex items-center mt-10 border-4 flex-col p-6">
	...
	{#if isFollowing == '' || (isFollowing == 'pending' && privateProfile)}
		<div>
			<p>This person is using technologies and such to stay private, so no more info for you.</p>
			<br />
			<p>You may try following him or her and when they accept, you might see something more interesting</p>
			<br />
